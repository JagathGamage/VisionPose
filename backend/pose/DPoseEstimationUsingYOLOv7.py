...
import sys
sys.path.append(f"{HOME}/pose/yolov7")
...
SOURCE_VIDEO_B_PATH = f"{HOME}/pose/input/vid-mid.mp4"
SOURCE_VIDEO_C_PATH = f"{HOME}/pose/input/vid-right.mp4"
...
try:
    ...
    frame = None  # Handle this case appropriately
...
# get_ipython().run_line_magic('cd', '{HOME}/pose/yolov7')
# get_ipython().system('wget https://github.com/WongKinYiu/yolov7/releases/download/v0.1/yolov7-e6e.pt --quiet')
# get_ipython().system('wget https://github.com/WongKinYiu/yolov7/releases/download/v0.1/yolov7-w6-pose.pt --quiet')
#
...
#detection_model.half()

# If you're using CUDA, ensure that the model is still transferred to the GPU
...
if not os.path.exists(SOURCE_VIDEO_A_PATH):
    raise FileNotFoundError(f"Error: Video file not found at {SOURCE_VIDEO_A_PATH}")

# Generate frames and check if the generator has frames
frame_iterator = iter(generate_frames(video_file=SOURCE_VIDEO_A_PATH))

try:
    frame = next(frame_iterator)
except StopIteration:
    raise RuntimeError("No frames available in the video. Check if the video file is valid and not empty.")

# Ensure the frame is valid
if frame is None or frame.size == 0:
    raise ValueError("Error: First frame is empty. Ensure the video is properly formatted.")
